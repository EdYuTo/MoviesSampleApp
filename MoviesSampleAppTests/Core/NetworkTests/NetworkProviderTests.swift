...
final class NetworkProviderTests: XCTestCase {
    ...
    func testMakeRequestWithRequestUrlError() async {
        do {
            _ = try await sut.makeRequest(.init(endpoint: "http://[::1"))
    ...
    func testMakeRequestWithConnectionErrors() async {
        ...
        for errorCode in connectionErrorCodes {
            do {
                ...
                _ = try await sut.makeRequest(.init(endpoint: #file))
                XCTFail("Error should be \(NetworkError.connectionError) but didn't throw")
            } catch {
                if case .connectionError = error as? NetworkError {} else {
                    XCTFail("Error should be \(NetworkError.connectionError) but is \(error)")
                }
            }
        }
    }
    ...
    func testMakeRequestWithDecodingError() async {
        do {
            ...
            let _: NetworkResponse<DecodableTest> = try await sut.makeRequest(.init(endpoint: #file))
            XCTFail("Error should be \(NetworkError.invalidParams) but didn't throw")
        } catch {
            if case .decodingError = error as? NetworkError {} else {
                XCTFail("Error should be \(String(describing: NetworkError.decodingError)) but is \(error)")
            }
        }
    }
}
